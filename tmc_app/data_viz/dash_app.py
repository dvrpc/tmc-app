...
def slider_marks():
    ...
    for h in range(0, 24):
        ...
        slider_marks[h] = {
            "label": f"{safe_h} {ampm}",
            "style": {"transform": "rotate(0deg)", "font-size": "70%"},
        }
    return slider_marks
...
def generate_treemap_data(df_timeseries, id_col: str = "location"):
    ...
    df_stacked = pd.DataFrame(df_timeseries.stack(), columns=["total"]).reset_index()
    ...
    # Explode the
    attribute_cols = {0: "veh_class", 1: "leg", 2: "movement"}
    df_attrs = (
        df_filtered["level_2"]
...
def create_dashboard(server):
    ...
    for project in all_projects:
        if project.files:
            project_options.append({"label": project.name, "value": project.uid})
        ...
        [
            html.Nav(
                [
                    html.A(
                        "Exit",
                        className="nav-item nav-link btn btn-outline-primary btn-sm",
                        href="/projects",
                    ),
                ],
            ...
            html.Div(
                [
                    ...
                    html.Div(
                        [
                            dcc.Graph(id="treemap-graph", figure=plot_tree),
                        ],
                        className="col-7",
                    ),
                ],
            ...
            html.Div(
                [
                    html.Div(
                        [
                            dcc.Graph(id="time-bar-graph", figure=plot_timeseries),
                        ],
                        className="col",
                    ),
                ],
...
def init_callbacks(dash_app):
    ...
    def change_project(pid, selected_range, mode_selector, treemap_path_order):
        ...
        plot_timeseries = timeseries_figure(
            df_timeseries, plot_kwargs=kwargs_timeseries
        )
